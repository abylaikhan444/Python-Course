...
# user_first_lst = input().split()
# user_second_lst = input().split()
# result = []
#
# for i in range(len(user_first_lst)):
#     s = int(user_first_lst[i]) + int(user_second_lst[i])
#     result.append(s)
#
# print(*result)
...
# if len(parts) == 3:
#     # Формат: abc-def-hijk
#     valid = len(parts[0]) == 3 and len(parts[1]) == 3 and len(parts[2]) == 4 and all(p.isdigit() for p in parts)
# elif len(parts) == 4:
#     # Формат: 7-abc-def-hijk (первая часть должна быть именно "7")
#     valid = parts[0] == '7' and len(parts[1]) == 3 and len(parts[2]) == 3 and len(parts[3]) == 4 and all(p.isdigit() for p in parts)
#
